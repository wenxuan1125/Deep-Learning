...
from utils import init_weights, kl_criterion, plot_pred, finn_eval_seq, pred, show_result
...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):
        # print(i)
        h_t = hidden[i][0]    # each elememt of hidden includes (hidden code, [4 feature maps])
        ...
        # print(f'cond shape: {cond[i-1].shape}')
        # print(f'h_t_1 shape: {h_t_minus_1.shape}')
        # print(f'z_t shape: {z_t.shape}')
        # print(f'cat shape: {torch.cat([cond[i-1], h_t_minus_1, z_t], 1).shape}')

        g_t = modules['frame_predictor'](torch.cat([cond[i-1], h_t_minus_1, z_t], 1))
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.betas = np.ones(args.niter) * 1.0
        self.i = 0
        ...
        if(args.kl_anneal_cyclical):
            ...
            for cycle in range(args.kl_anneal_cycle):
                beta, i = 0.0, 0.0
                while(beta <= 1.0 and int(i + cycle * period) < args.niter):
                    self.betas[int(i + cycle * period)] = beta
                    beta += step
                    i += 1
        else: 
            period = args.niter 
...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f-niter=%d-tfr_start_decay_epoch=%d'\
            % (args.rnn_size, args.predictor_rnn_layers, args.posterior_rnn_layers, args.n_past, args.n_future, args.lr, args.g_dim, args.z_dim, args.last_frame_skip, args.beta, args.niter, args.tfr_start_decay_epoch)
        name += '-kl_cyclical=' + str(args.kl_anneal_cyclical)
    ...
    ##
    tfr_start = args.tfr    # tfr -> teacher forcing ratio
    ## 
    print_loss = []
    print_mse = []
    print_kld = []
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        print_loss.append(epoch_loss  / args.epoch_size)
        print_mse.append(epoch_mse / args.epoch_size)
        print_kld.append(epoch_kld / args.epoch_size)
        print_tfr.append(args.tfr)
        print_beta.append(kl_anneal.get_beta())
        
        # Update beta
        kl_anneal.update()
        if epoch >= args.tfr_start_decay_epoch:
            ### Update teacher forcing ratio ###
            # decay_step = (tfr_start - args.tfr_lower_bound) / (niter - args.tfr_start_decay_epoch)
            # args.tfr -= decay_step
            # args.tfr = max(args.tfr_lower_bound, args.tfr)
            # # print(f'TRF: {args.tfr}')
        ...
        if epoch % 5 == 0 or epoch == start_epoch+niter-1:
            psnr_list = []
            for _ in range(len(validate_data) // args.batch_size):  # a / b = c ... d, a // b = c
                try:
                ...
                pred_seq = pred(validate_seq, validate_cond, modules, epoch, args, device)
            ...
            if ave_psnr > best_val_psnr:
                ...
                torch.save({
                    ...
                    'args': args,
                    'last_epoch': epoch},
                    '%s/model_%d_%f.pth' % (args.log_dir, epoch, ave_psnr))
    ...
    show_result(niter, print_loss, print_mse, print_kld, print_tfr, print_beta, print_psnr, args)
