...
def ddpm_schedules(beta1, beta2, T):
    ...
    # beta_t = (beta2 - beta1) / 2 * torch.cos(2 * math.pi * torch.arange(0, T + 1, dtype=torch.float32) / 50) + (beta2 + beta1) / 2
...
class DDPM(nn.Module):
    ...
    def forward(self, x, c):
        ...
        context_mask = torch.bernoulli(torch.zeros_like(c)+self.drop_prob).to(self.device) # context_mask.shape = (128, 24)
